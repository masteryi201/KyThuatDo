...
join_path = os.path.join(mypath, "..\\Modules\\ThietKe_NhapKhauGiam.py")
...
mypath = Path().absolute()
join_path = os.path.join(mypath, "..\\\Temp\\\ThietKe_ThongSoKhauGiam.py")
...
if check_true == 1 :
	for j in range(1, number+1):
		...
		if self.ChonKhauBoiThuong%s.isChecked() == True:
			...
			value_checkbox.write('KhauBoiThuongGiam = %s\\n')
